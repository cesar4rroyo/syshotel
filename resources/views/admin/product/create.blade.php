...
<div class="flex space-x-6">
    <div class="flex flex-col space-y-1 w-full">
        ...
        <input class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-0 focus:border-gray-300 focus:outline-none block w-full px-4 py-2.5" type="text" name="name" id="name"
            value="{{ isset($formData['model']) ? $formData['model']->product->name : null }}" required>
    </div>
</div>
<div class="flex space-x-6">
    <div class="flex flex-col space-y-1 w-full">
    ...
    <div class="flex flex-col space-y-1 w-full">
        ...
        <select
            ...
            @foreach ($formData['cboCategory'] as $key => $value)
                <option value="{{ $key }}"
                    {{ isset($formData['model']) && $formData['model']->product->category_id == $key ? 'selected' : null }}>
                    {{ $value }}
                </option>
            @endforeach
        </select>
    ...
    <div class="flex flex-col space-y-1 w-full">
        ...
        <select
            ...
            @foreach ($formData['cboUnit'] as $key => $value)
                <option value="{{ $key }}"
                    {{ isset($formData['model']) && $formData['model']->product->unit_id == $key ? 'selected' : null }}>
                    {{ $value }}
                </option>
            @endforeach
        </select>
