<div class="w-full py-8 px-12 rounded-xl bg-white">
    <button style="width: 120px; float:right"
        class="text-white bg-blue-500 hover:bg-yellow-500 focus:outline-none focus:ring-0 font-medium rounded-lg text-sm px-3.5 py-2.5 flex items-center space-x-2 mb-4"
        id="btnNuevo" onclick="cargarRuta('{{ URL::to($routes['back']) }}', 'main-container')">
    ...
    <div class="flex space-x-5 mt-5">
        <div class="flex flex-col space-y-1 w-full">
            <label class="font-medium text-sm text-gray-600"
                for="number">{{ trans('maintenance.control.management.number') }}</label>
            <input
                class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-0 focus:border-gray-300 focus:outline-none block w-full px-4 py-2.5 bg-blue-100"
                type="text" name="number" id="number" readonly value="{{ $model->number }}" required>
        </div>
        <div class="flex flex-col space-y-1 w-full">
            <label class="font-medium text-sm text-gray-600"
                for="date">{{ trans('maintenance.control.management.date') }}</label>
            <input
                class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-0 focus:border-gray-300 focus:outline-none block w-full px-4 py-2.5 bg-blue-100"
                type="date" name="date" id="date" readonly value="{{ $model->date }}" required>
        </div>
        <div class="flex flex-col space-y-1 w-full">
            <label class="font-medium text-sm text-gray-600"
                for="start_date">{{ trans('maintenance.control.management.start_date') }}</label>
            <input
                ...
                type="datetime-local" name="start_date" id="start_date" value="{{ $model->start_date }}" required {{ $model->status == config('constants.processStatus.PyC') ? 'readonly' : null}}>
        </div>
        <div class="flex flex-col space-y-1 w-full">
            <label class="font-medium text-sm text-gray-600"
                for="end_date">{{ trans('maintenance.control.management.end_date') }}</label>
            <input
                class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-0 focus:border-gray-300 focus:outline-none block w-full px-4 py-2.5"
                type="datetime-local" name="end_date" id="end_date" value="{{ $model->end_date ?? $today }}" required {{ $model->status == config('constants.processStatus.PyC') ? 'readonly' : null}}>
        </div>
    </div>
    <div class="flex space-x-6 mt-3">
        <div class="flex flex-col space-y-1 w-full">
            <label class="font-medium text-sm text-gray-600"
                for="room">{{ trans('maintenance.control.management.room') }}</label>
            <input
                class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-0 focus:border-gray-300 focus:outline-none block w-full px-4 py-2.5 bg-blue-100"
                type="text" name="room" id="room" value="{{ $model->room->name ?? $room->name }}" readonly>
        </div>
        <div class="flex flex-col space-y-1 w-full">
            <label class="font-medium text-sm text-gray-600"
                for="price">{{ trans('maintenance.control.management.room') }}</label>
    ...
    <div class="flex space-x-6 mt-3">
        <div class="flex flex-col space-y-1 w-full">
            ...
            <select
                class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-0 focus:border-gray-300 focus:outline-none block w-full px-4 py-2.5"
                name="client_id" id="client_id" {{ $model->status == config('constants.processStatus.PyC') ? 'disabled' : null}}>
                @foreach ($cboClients as $key => $value)
                    <option value="{{ $key }}" {{ $model->client_id == $key ? 'selected' : null }}>
                        {{ $value }}</option>
                @endforeach
            </select>
        </div>
    ...
    <div class="flex items-center justify-end space-x-5 py-3 w-full">
        <button class="px-5 py-2 rounded-lg bg-blue-corp text-white flex items-center space-x-2" id="btnGuardar"
            ...
            <p>{{ __('maintenance.control.check-out') }}</p>
        </button>
    </div>
</div>
</form>
...
<script type="text/javascript">
    ...
    function guardar(entidad, idboton, entidad2) {
        ...
        var data = submitForm(idformulario, 'PUT');
        var respuesta = '';
        var listar = 'NO';
